...
//------------------------------------------------------------------------------------------------
            ...
            //  */
            // std::vector<std::vector<std::pair<std::shared_ptr<mata::nfa::Nfa>, mata::strings::seg_nfa::VisitedEpsilonsCounterVector>>> newNoodles;
            // for (size_t pivot = 0; pivot < noodles.size(); pivot++){
            //     // load the pivot-th noodle into variable udon
            //     auto udon = noodles[pivot];
            //     bool push_udon = true;
            //     size_t newNoodles_size = newNoodles.size();
            //     // loop through all "sorted" noodles
            //     for (size_t j = 0; j < newNoodles_size; j++){
            //         auto soba = newNoodles[j];
            //         size_t l = 0;
            //         // Now the sizes of the udon.first and soba.first do not have to be the same so how do we compare them?
            ...
            //                     // they are not the same, we can push it to newNoodles
            //                     STRACE("str", tout << "soba and udon are not the same" << std::endl;);
            //                     break; // eeeeeeeeeeeeeeeeeeh
            //                 }
            //             }

            //             STRACE("str", tout << "*l: "+ToString(l)+", bigger_nood.size(): " +ToString(bigger_nood.size()) << std::endl;);

            //             l++;
            //         }
            //         // we need to revert the last increment of 1
            //         l--;
            //         STRACE("str", tout << "l: "+ToString(l)+", bigger_nood.size(): " +ToString(bigger_nood.size()) +" smaller_nood.size(): "+ToString(smaller_nood.size()) << std::endl;);
            //         while (l != bigger_nood.size() - 1 && bigger_nood[l].first->final == bigger_nood[l].first->initial)
            //                     l++;
